    ...
    <div className="w-full group">
      ...
      >
        ...
        <div className={`${draft ? "secondary" : "bg-neutral-50 dark:bg-neutral-950"} relative z-10 w-fit flex flex-col`} style={{ padding: state.padding, gap: state.padding }}>
          <p
            className={`uppercase font-semibold font-Franklin leading-tight transition-all duration-230 ${draft ? "text-neutral-50 dark:text-neutral-950 opacity-100" : ""}`}
            style={{ fontSize: state.fontSize }}
          >
            {title}
          </p>
          <p
            className={`hidden md:block italic font-Gentium transition-all duration-230 ${draft ? "text-neutral-50 dark:text-neutral-950 opacity-100" : "opacity-60"}`}
        ...
        <div className="font-semibold flex gap-2 pt-2 transition-all duration-230 relative z-10">
          {tags?.map((tag, index) => (
            <p
              className={`${draft ? "secondary !text-neutral-50 dark:!text-neutral-950 opacity-100" : "primary"} transition-all duration-230`}
              key={index}
              style={{
